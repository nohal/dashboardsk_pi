...
SignalK context of the own vessel. The initial *vessels.* must be ommited. The value is normally filled automatically as soon as it is received from the OpenCPN core application and there is no need to edit it manually.
...
=== SignalK data tree viewer (3)

The data tree constructed from the received SignalK delta messages can be displayed as a collapsible tree structure by clicking on the *SignalK* button.

image::data_tree.png[SignalK data tree viewer]
...
This allows an immediate inspection of the effect of the configuration changes performed, but the configurstion is persisted and permanently applied only after the *OK* button in the lower right corner of the preferences is pressed.
