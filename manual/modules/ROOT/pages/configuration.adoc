...
When the edited data is applied, and whenever a configuration, dashboard or instrument is imported from a file, the JSON is validated against the plugin configuration schema. If it does not match, the change is rejected and the reported problems are shown, leaving the current configuration untouched.
...
|Anchor
|Edge of the screen to which the dashboard is docked. The *Own ship* option
centers the dashboard's instruments in a horizontal row on OpenCPN's own-ship
position. The dashboard is hidden when the position is unavailable or outside
the canvas.

|Offset h
|Horizontal offset from the edge of the screen. Ignored for the *Own ship*
anchor.

|Offset v
|Vertical offset from the edge of the screen. Ignored for the *Own ship*
anchor.
...
|Spacing v
|Vertical spacing between the instruments in the dashboard. Ignored for the
*Own ship* anchor.
|===
